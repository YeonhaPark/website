    ...
    <GradientTemplate>
      <main>
        <section id={"about"} className={"my-20 w-10/12 md:w-full mx-auto"}>
          ...
          <div>
            {about.map((el: string, idx: number) => (
        ...
        <section id={"skills"} className={"mb-20 w-10/12 md:w-full mx-auto"}>
          ...
          <div>
            <ul>
              {skills.map((el) => (
                <li key={el.title} className={"mb-2"}>
                  <div>
                    <h3 className={"mb-2 font-semibold text-sm"}>{el.title}</h3>
                    <p>{el.description}</p>
                  </div>
                </li>
              ))}
        ...
        <section
          id={"experience"}
          className={"mb-20 w-10/12 md:w-full mx-auto"}
        >
          <div className={"text-lg md:text-xl font-semibold mb-4"}>
            <h2>Experience</h2>
          </div>
          <div>
            <ul>
              {workExperiences.map((el) => (
                <li key={el.id} className={"mb-2"}>
                  <div
                    className={
                      "group relative grid mb-10 transition-all sm:grid-cols-8 sm:gap-8 md:gap-4 lg:hover:!opacity-100 lg:group-hover/list:opacity-50"
                    }
                  >
                    <header className={"text-sm mt-1"}>{el.duration}</header>
                    <div className={"sm:col-span-6"}>
                      <div className="flex gap-3 items-center mb-1">
                        <h3 className={"font-semibold"}>{el.pos}</h3>
                        {el.link && (
                          <Link
                            href={el.link}
                      ...
                      <pre
                        className={
                          "text-sm md:text-base mb-5 whitespace-pre-wrap"
                        }
                      >
                        {el.description}
                      </pre>
            ...
            <ul>
              {education.map((el) => (
                <li
                  key={el.title}
                  className={
                    '"group mb-2 relative grid pb-1 transition-all sm:grid-cols-8 sm:gap-8 md:gap-4 lg:hover:!opacity-100 lg:group-hover/list:opacity-50"'
                  }
                >
                  <header className={"text-sm mt-1"}>{el.year}</header>
