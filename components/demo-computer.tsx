...
export type DemoComputerProps = JSX.IntrinsicElements["group"] & {
  /** URL or HTMLVideoElement for the monitor texture (video). */
  texture?: string;
  /** Scale multiplier for the internal scene (default: 1.5) */
  size?: number;
};
...
export function DemoComputer({
  ...
  const isCustom = !!texture;
  const videoTexture = useVideoTexture(
    isCustom ? texture! : "/textures/project/project1.mp4"
  ); // three/drei returns a new VideoTexture when the source changes

  // Keep the active texture in state so React re-renders the material when it changes
  const [currentTexture, setCurrentTexture] =
    useState<THREE.VideoTexture | null>(videoTexture);

  // Keep a ref to the previous texture so we can dispose it after React updates
  const prevTextureRef = useRef<THREE.VideoTexture | null>(null);

  // Ensure flipY is correct on the live texture
  videoTexture.flipY = false;

  // When a new texture is produced by useVideoTexture, swap it into state
  useEffect(() => {
    // record previous before updating state
    prevTextureRef.current = currentTexture;
    setCurrentTexture(videoTexture);
    // attempt to play the underlying video element (may be auto-play blocked by browser)
    try {
      const img = videoTexture.image;
      if (img && typeof img.play === "function") {
        // some browsers require a user gesture to play with sound; typically these videos are muted
        // ignore play rejection
        void img.play().catch(() => {});
      }
    } catch (e) {
      // non-fatal if play fails
      console.error("Error attempting to play video texture:", e);
    }
  }, [texture, videoTexture]);

  // Dispose previous texture after the currentTexture has updated and rendered
  useEffect(() => {
    const prev = prevTextureRef.current;
    if (prev && prev !== currentTexture) {
      try {
        prev.dispose();
      } catch (error) {
        console.error("Error disposing previous texture:", error);
      }
      prevTextureRef.current = null;
    }
  }, [currentTexture]);

  // Animate Scene scale from 1 -> size on mount (and when `size` changes)
  useEffect(() => {
    ...
    // Animate to the requested size for x/y/z
  ...
  useGSAP(() => {
    ...
    // animate from a rotated side view to facing forward (y = 0)
    gsap.fromTo(
  ...
  return (
    <group
      ref={group}
      {...(props as JSX.IntrinsicElements["group"])}
      dispose={null}
    >
      <group name="Scene" ref={sceneRef} scale={[1, 1, 1]}>
        <mesh
          name="monitor-screen"
          castShadow
          receiveShadow
