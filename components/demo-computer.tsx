...
export function DemoComputer({ texture, ...props }: DemoComputerProps) {
  ...
  ); // videoTexture is generated each render, so we need to manage its disposal
  const textureRef = useRef<THREE.VideoTexture>(videoTexture);
  videoTexture.flipY = false;

  useEffect(() => {
    const old = textureRef.current;
    textureRef.current = videoTexture;
    if (old && old !== textureRef.current) {
      try {
        old.dispose();
      } catch (error) {
        console.error("Error disposing old texture:", error);
      }
    }
